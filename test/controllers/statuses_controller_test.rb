...
class StatusesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create status" do
    assert_difference('Status.count') do
      post statuses_url, params: { status: { content: @status.content, name: @status.name, text: @status.text } }
    end
  ...
  test "should update status" do
    patch status_url(@status), params: { status: { content: @status.content, name: @status.name, text: @status.text } }
